...
{{-- 1. Sidebar (Menu Samping) --}}
    <aside id="sidebar" class="sidebar fixed inset-y-0 left-0 z-40 flex flex-col w-64 bg-gray-800 text-white shadow-xl md:relative md:translate-x-0">
        ...
        <header class="h-16 flex items-center justify-between px-4 bg-red-600 shadow-lg">
            <h1 class="text-xl font-bold tracking-tight truncate uppercase">{{ Str::title(str_replace('_', ' ', (Auth::user()->getRoleNames()->first()))) }}</h1>
        ...
        <nav class="sidebar-content flex-grow overflow-y-auto pt-2 pb-4 space-y-1">
            ...
            @role('admin')
            <x-side-style href="{{ route('admin.dashboard') }}" :active="request()->is('dashboard')">
            ...
            <x-side-style {{-- href="{{ route('nilai.index') }}" :active="request()->is('nilai.index')" --}}>
                <i data-lucide="calendar" class="w-5 h-5 mr-3"></i>
                <span>Master Penilaian ATT</span>
            </x-side-style>
            ...
            @role('walikelas')
            <x-side-style href="{{ route('walikelas.dashboard') }}" :active="request()->is('dashboard')">
                <i data-lucide="users" class="w-5 h-5 mr-3"></i>
            ...
            <x-side-style href="{{ route('walikelas.dashboard') }}" :active="request()->is('dashboard')">
                <i data-lucide="users" class="w-5 h-5 mr-3"></i>
                <span>Milai Siswa</span>
            </x-side-style>

            <x-side-style href="{{ route('walikelas.dashboard') }}" :active="request()->is('dashboard')">
                <i data-lucide="users" class="w-5 h-5 mr-3"></i>
                <span>Histori Perkembangan</span>
            </x-side-style>
            ...
            @role('guru_att')
            <x-side-style href="{{ route('guru_att.dashboard') }}" :active="request()->is('*.dashboard')">
                <i data-lucide="users" class="w-5 h-5 mr-3"></i>
                <span>Dashboard Guru ATT</span>
            </x-side-style>

            <x-side-style {{-- href="{{ route('att.index') }}" :active="request()->is('att.index')" --}}>
                <i data-lucide="calendar" class="w-5 h-5 mr-3"></i>
                <span>Input nilai ATT</span>
            </x-side-style>
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="flex items-center py-2 px-4 text-sm font-medium hover:bg-gray-700 rounded-lg mx-2 transition-colors">
